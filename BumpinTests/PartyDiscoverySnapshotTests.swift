...
final class PartyDiscoverySnapshotTests: XCTestCase {
    ...
    func test_discover_empty_snapshot() {
        struct EmptyShim: View {
            var body: some View {
                PartyDiscoveryView()
                    .onAppear { /* no data; shows empty states per tab */ }
            }
        }
        assertSnapshotHashEquals(EmptyShim(), expectedHash: "REPLACE_WITH_BASELINE_DISCOVER_EMPTY", label: "discover_empty")
    }

    func test_discover_loading_snapshot() {
        struct LoadingShim: View {
            @StateObject private var mgr = PartyDiscoveryManager()
            var body: some View {
                VStack {
                    // Simulate loading state
                    PartyDiscoveryView()
                }
                .onAppear { mgr.isLoading = true }
            }
        }
        assertSnapshotHashEquals(LoadingShim(), expectedHash: "REPLACE_WITH_BASELINE_DISCOVER_LOADING", label: "discover_loading")
    }
}
