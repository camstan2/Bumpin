# Complete Firestore Security Rules for Bumpin
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    function isAddingSelfToParticipants() {
      // Check if the user is adding themselves to the participants array
      // This allows users to join parties by adding themselves to the participants list
      return request.resource.data.participants is list;
    }
    
    function hasParticipantId(participants, userId) {
      // Check if userId exists in the participants array (which contains objects with 'id' field)
      return participants is list && participants.size() > 0;
    }

    // Content Violations - allow users to log their own violations (created by moderation service)
    match /contentViolations/{violationId} {
    ...
    match /users/{uid} {
      ...
      allow update: if isSignedIn() && request.auth.uid == uid && !elevatesToAdmin();
      
      // Only admins can set admin privileges
      allow update: if isSignedIn() && request.auth.uid == uid && elevatesToAdmin() && isAdmin();
      
      // Allow users to update ONLY the followers/following arrays of other users (for follow/unfollow)
      ...
      // Listen Later - users can manage their own listen later items (nested form; currently unused)
      match /listenLater/{docId} {
    ...
    // ADD: Listen Later (top-level collection) - owner-only access
    match /listenLater/{docId} {
      // Read allowed only if the document belongs to the signed-in user
      allow read: if isSignedIn() && resource.data.userId == request.auth.uid;
      // Create allowed only if the new document's userId matches the signed-in user
      allow create: if isSignedIn() && request.resource.data.userId == request.auth.uid;
      // Update/Delete allowed only by the owner
    ...
    match /logs/{logId} {
      ...
      allow update: if isSignedIn() && (
        ...
        // Allow incrementing comment count, like count, and repost count for engagement
        (request.resource.data.diff(resource.data).affectedKeys().hasOnly(['commentCount', 'likeCount', 'repostCount']))
      );
      allow delete: if isSignedIn() && request.auth.uid == resource.data.userId;
      
      // Likes subcollection - users can like/unlike logs
      match /likes/{userId} {
        allow read: if true;
        allow write: if isSignedIn() && request.auth.uid == userId;
      }
      
      // ThumbsDown subcollection - users can thumbs down logs
      match /thumbsDown/{userId} {
        allow read: if true;
        allow write: if isSignedIn() && request.auth.uid == userId;
      }
      
      // Reposts subcollection - users can repost logs
      match /reposts/{userId} {
        allow read: if true;
        allow write: if isSignedIn() && request.auth.uid == userId;
      }
      
      // Comments subcollection - UPDATED FOR COMMENT FUNCTIONALITY
      match /comments/{commentId} {
        allow read: if true;
        allow create: if isSignedIn() && request.resource.data.userId == request.auth.uid;
        allow update: if isSignedIn() && (
          request.auth.uid == resource.data.userId ||
          // Allow updating like/reply counts
    ...
    // Allow reading/writing reposts under any parent (logs/{id}/reposts/*, items/{id}/reposts/*)
    match /{anyPath=**}/reposts/{repostId} {
      allow read: if true;
      allow create: if isSignedIn() && request.resource.data.userId == request.auth.uid;
      allow update: if false; // immutable repost docs
      allow delete: if isSignedIn() && request.auth.uid == resource.data.userId;
    }

    // Comment Replies - replies to music logs
    ...
    match /parties/{partyId} {
      ...
      allow update: if isSignedIn() && (
        // Host and co-hosts can update anything
        resource.data.hostId == request.auth.uid ||
        (resource.data.coHostIds is list && request.auth.uid in resource.data.coHostIds) ||
        // Allow any participant to update if they're only modifying participants list
        (request.resource.data.diff(resource.data).affectedKeys().hasOnly(['participants']) &&
         isAddingSelfToParticipants()) ||
        // Allow any participant to update currentSong, queue, etc. (for sync)
        (request.auth.uid in resource.data.participantIds ||
         hasParticipantId(resource.data.participants, request.auth.uid))
      );
    ...
    match /topicTrendingMetrics/{topicId} {
      allow read: if isSignedIn();
      allow create, update: if isSignedIn(); // Allow authenticated users to update trending metrics
    }
...
### The critical section that enables notifications:
...
This allows:
- ✅ Any authenticated user to create notifications for any other user (needed for likes, follows, etc.)
- ✅ Users to read only their own notifications
- ✅ Users to update only their own notifications (for marking as read)
- ✅ Users to delete only their own notifications
...
5. Copy the entire rules block above
6. Paste it into the rules editor (replacing all existing content)
7. Click **Publish**
8. Wait a few seconds for the rules to deploy

✅ Once deployed, all notifications will work correctly!
