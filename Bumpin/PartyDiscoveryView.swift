...
struct PartyDiscoveryView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onDisappear {
                discoveryManager.stopDiscovery()
                AnalyticsService.shared.logDiscoveryLifecycle(event: "pause", tab: tabName(selectedTab))
            }
            .onChange(of: scenePhase) { _, newPhase in
                switch newPhase {
                case .active:
                    Task { await discoveryManager.resumeIfNeeded(locationManager: locationManager) }
                    AnalyticsService.shared.logDiscoveryLifecycle(event: "resume", tab: tabName(selectedTab))
        ...
        ScrollView {
            LazyVStack(spacing: 16) {
                    ...
                    PartyCard(party: party, currentLocation: locationManager.currentLocation, onJoin: {
                                    discoveryManager.quickJoinParty(party)
                                }, isNearbyParty: false, isExploreParty: false)
                            }
            }
            .padding(.horizontal, 16)
            .padding(.vertical, 16)
...
struct PartyCard: View {
    ...
    let isExploreParty: Bool
    @State private var showingPreview = false
    @State private var profileTapped = false
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Button(action: onJoin) {
                Text("Join")
                    ...
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 12)
                    .background(party.isFriendsParty ? Color.green : (isNearbyParty ? Color.yellow : (isExploreParty ? Color.red : Color.blue)))
                    .cornerRadius(8)
            }
        }
        .padding()
        ...
        .onTapGesture {
            ...
            profileTapped = false // Reset for next tap
        }
...
class PartyDiscoveryManager: ObservableObject {
    ...
    func loadMoreFollowing(step: Int = 20) {
        ...
        AnalyticsService.shared.logDiscoveryLoadMore(tab: "following", page: followingDisplayCount / step)
    ...
    func loadMoreFriends(step: Int = 20) {
        ...
        AnalyticsService.shared.logDiscoveryLoadMore(tab: "friends", page: friendsDisplayCount / step)
    ...
    func startDiscovery(locationManager: LocationManager) async {
        ...
        AnalyticsService.shared.logTiming(event: "discover_initial_explore_load", time: Date().timeIntervalSince(t0))
    }
    ...
    func loadMoreExplore() async {
        ...
        do {
            ...
            AnalyticsService.shared.logDiscoveryLoadMore(tab: "explore", page: exploreParties.count / 20)
        } catch {
    ...
    private func setupFirestoreListeners() {
        ...
        let nearbyListener = nearbyQuery.addSnapshotListener { [weak self] snapshot, error in
            ...
            AnalyticsService.shared.logTiming(event: "discover_listener_nearby_first_snapshot", time: Date().timeIntervalSince(nearbyAttachT0))
                        ...
                        let listener = db.collection("parties")
                            ...
                            .addSnapshotListener { snapshot, err in
                                if let err = err {
                                    ...
                                    AnalyticsService.shared.logDiscoveryRetry(tab: key, attempt: attempt)
                                ...
                                AnalyticsService.shared.logTiming(event: "discover_listener_following_batch_first_snapshot", time: Date().timeIntervalSince(attachT0), parameters: ["batch": index])
                    ...
                    func attach(_ manager: PartyDiscoveryManager) {
                        ...
                        let listener = db.collection("parties")
                            ...
                            .addSnapshotListener { snapshot, err in
                                ...
                                AnalyticsService.shared.logTiming(event: "discover_listener_friends_batch_first_snapshot", time: Date().timeIntervalSince(attachT0), parameters: ["batch": index])
                                let docs = snapshot?.documents ?? []
        ...
        let exploreListener = firestore.collection("parties")
            ...
            .addSnapshotListener { [weak self] snapshot, error in
                ...
                AnalyticsService.shared.logTiming(event: "discover_listener_explore_first_snapshot", time: Date().timeIntervalSince(exploreAttachT0))
