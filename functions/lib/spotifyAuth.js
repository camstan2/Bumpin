...
exports.getSpotifyAuthUrl = exports.refreshSpotifyToken = exports.exchangeSpotifyCode = exports.getSpotifyClientToken = void 0;
...
async function makeSpotifyTokenRequest(body, config) {
 ...
 */
exports.getSpotifyClientToken = functions.https.onCall(async (data, context) => {
    try {
        // Authenticate user
        await authenticate(context);
        const config = getSpotifyConfig();
        // Make token request
        const tokenData = await makeSpotifyTokenRequest('grant_type=client_credentials', config);
...
exports.exchangeSpotifyCode = functions.https.onCall(async (data, context) => {
    try {
        ...
        const tokenData = await makeSpotifyTokenRequest(`grant_type=authorization_code&code=${encodeURIComponent(code)}&redirect_uri=${encodeURIComponent(redirectUri)}`, config);
...
exports.refreshSpotifyToken = functions.https.onCall(async (data, context) => {
    try {
        ...
        const config = getSpotifyConfig();
        // Refresh the token
        const tokenData = await makeSpotifyTokenRequest(`grant_type=refresh_token&refresh_token=${encodeURIComponent(refreshToken)}`, config);
 ...
 */
exports.getSpotifyAuthUrl = functions.https.onCall(async (data, context) => {
    try {
        // Authenticate user
        await authenticate(context);
